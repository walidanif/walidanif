...
/*------------------------------------------
    ...
    if($(".testimonial-silder".length)) {
       ...
       //  hover-active
       let items = document.querySelectorAll('.blog-single-item');
    ...
    if ($("#contact-form-main").length) {
        $("#contact-form-main").validate({
            ...
            messages: {
                ...
                note: "Please type your contact textarea"
            },
    ...
    $(document).on('click','.click-modal-service',function(){
        const title = $(this).data('title')
        const image = $(this).data('image')
        const imageon = $(this).data('imageon')
        const imagetwo = $(this).data('imagetwo')
        $('.modal-title').text(title)
        $('.modal-img').attr('src',image)
        $('.details-img-1').attr('src',imageon)
        $('.details-img-2').attr('src',imagetwo)
    })

    $(document).on('click','.click-modal-blog',function(){
        const title = $(this).data('title')
        const image = $(this).data('image')
        const imageon = $(this).data('imageon')
        const imagetwo = $(this).data('imagetwo')
        $('.modal-title').text(title)
        $('.entry-media-img').attr('src',image)
        $('.gallery-1').attr('src',imageon)
        $('.gallery-2').attr('src',imagetwo)
    })



    $('.term-list').each(function(){
        var LiN = $(this).find('.col').length;
        if( LiN > 3){    
          $('.col', this).eq(3).nextAll().hide().addClass('toggleable');
          $(this).append('<div class="theme-btn-s2 s2">see all services</div>');    
        }  
      });
      $('.term-list').on('click','.theme-btn-s2.s2', function(){
        if( $(this).hasClass('less services') ){    
          $(this).text('see all services').removeClass('less services');    
        }else{
          $(this).text('less services').addClass('less services'); 
        }
        $(this).siblings('.col.toggleable').slideToggle(); 
      }); 
